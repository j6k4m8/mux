...
const DEMO_THREADS: &[DemoThread] = &[
    DemoThread {
        ...
        snippet: "Let's do that. Priya, if you take the supplement paragraph I'll fix the numbering",
        category: "Work",
        in_inbox: 1,
        unread: 1,
        starred: 1,
    ...
    DemoThread {
        ...
        snippet: "You left the good tupperware here. It has your name on it in sharpie, which I respect.",
        category: "Personal",
        in_inbox: 1,
        unread: 0,
        starred: 0,
    ...
    DemoThread {
        ...
        snippet: "A technician will service the radiator in your unit on Thursday between 9am and 12pm.",
        category: "Personal",
        in_inbox: 1,
        unread: 0,
        starred: 0,
...
fn seed_long_demo_threads(connection: &mut Connection) -> Result<(), StoreError> {
    ...
    let scenario_count: i64 = connection.query_row(
        "SELECT COUNT(*) FROM threads WHERE id = 5",
        [],
        |row| row.get(0),
    )?;
...
mod tests {
    ...
    fn the_seeded_inbox_is_four_written_conversations_and_no_filler() {
        ...
        for filler in [
            ...
            assert!(!bodies.contains(filler), "templated filler returned: {filler}");
        }
    ...
    fn local_snooze_and_rsvp_journals_are_undoable_but_reject_stale_undo() {
        ...
        let first_rsvp = store.rsvp_thread(DEMO_FIXTURE_THREAD_ID, "accepted").unwrap();
        let second_rsvp = store.rsvp_thread(DEMO_FIXTURE_THREAD_ID, "tentative").unwrap();
        assert!(matches!(
    ...
    fn activity_is_bounded_and_excludes_payloads_and_human_error_text() {
        ...
        let mut store = MuxStore::open(&path, true).expect("seeded store");
        store.rsvp_thread(DEMO_FIXTURE_THREAD_ID, "accepted").unwrap();
        store.rsvp_thread(DEMO_FIXTURE_THREAD_ID, "tentative").unwrap();
        let one = store
