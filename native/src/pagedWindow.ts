...
/// Collect a stable, deduplicated page window without committing partial
/// results. Returning null tells the caller that navigation made this request
/// stale.
export async function collectPagedWindow<T extends Identified>({
  minimumRows,
  initialRows = [],
  initialCursor = null,
  initialHasMore = true,
  ...
  const rows = new Map<string | number, T>(initialRows.map((row) => [row.id, row]));
...
export function normalizeMailboxWindowStart(
  start: number,
  rowCount: number,
...
export function mailboxRenderWindow<T>(
  rows: T[],
  start: number,
  size = MAILBOX_RENDER_WINDOW_SIZE
...
export function mailboxWindowStartForIndex(
  ...
  if (!Number.isInteger(index) || index < 0 || index >= rowCount) {
    return normalizeMailboxWindowStart(currentStart, rowCount, size);
  }
  const start = normalizeMailboxWindowStart(currentStart, rowCount, size);
...
export async function recoverRequiredRow<T extends Identified>({
  page,
  requiredId,
  fetchRequired,
  isCurrent,
...
export function threadPageOrder(
  left: { id: number; latestAt: number },
  right: { id: number; latestAt: number }
): number {
